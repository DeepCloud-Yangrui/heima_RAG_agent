...
# 2、设置好示例数据、提问数据
# schema里存放你希望从文本中提取的哪些目标字段
schema = ['日期', '股票名称', '开盘价', '收盘价', '成交量']


# 示例数据：examples_data是一个列表，存放两份示例数据
examples_data = [
    {
...
# 这里的具体过程和上一个金融文本分类任务差不多
# 3、用messages来封装一下模型的角色，messages是一个列表，列表中的元素是字典
messages = [
    {"role":"system","content":f"你帮我完成信息抽取，我给你句子，你抽取{schema}信息，按JSON字符串输出，如果某些信息不存在，用'原文未提及'表示，请参考如下示例："}
]

print(type(examples_data))

# 4、添加示例信息
for example in examples_data:
    messages.append({"role":"user","content":example["content"]})
    # 这里为什么要用json.dumps(example["answers"],ensure_ascii=False)？
    # OpenAI API 规定 messages 列表中的 content 字段的值必须是纯字符串（str），不能直接传入 Python 的字典（dict）对象
