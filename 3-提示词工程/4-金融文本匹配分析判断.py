...
# 4、添加示例信息
for example_key in examples_data.keys():
    for example_value in examples_data[example_key]:
        # 测试：尝试用两层for循环来拿到example_data中的所有数据
        # print(f"{example_key}---{example_value}")
        messages.append({"role": "user", "content": f"{example_value}"})
        messages.append({"role": "assistant", "content": f"{example_key}"})
...
for q in questions:
    response = client.chat.completions.create(
        model="qwen3.8-max",
        messages=messages + [{"role": "user", "content": f"按照上述示例，回答这2个句子的情况:{q}"}]
    )
